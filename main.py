    ...
    def on_ws_private_data(self,data):
        ...
        if data["type"] == "myOrder":
            ...
            if state != "done":
                ...
                volume = format(volume, '.8f')
                volume = float(volume)
                remaining_volume = data["remaining_volume"] # 체결 후 아직 미체결된 주문 양
                remaining_volume = format(remaining_volume, '.8f')
                ...
                if state != "cancel":
                    ...
                    if state == "trade":
                        ...
                        if uuid in Upbit.not_balance_dict.keys():
                            print("trade__",uuid,Upbit.not_balance_dict,remaining_volume,volume)
                            if remaining_volume == 0 and volume > 0:
                               
                                Upbit.not_balance_dict.pop(uuid)

                                if (code in Upbit.buy_list) and (ask_bid == "BID"):
                                    Upbit.buy_list.remove(code)

                                elif (code in Upbit.sell_list) and (ask_bid == "ASK"):
                                    Upbit.sell_list.remove(code)
        ...
        for _symbol in self.symbol:
            ...
            if Upbit.매수 == False:
                ...
                if close < ema12 and ema12 < ema26:
                    ...
                    if _symbol in Upbit.balance_dict.keys() and _symbol not in Upbit.sell_list:
                        _sAmount = Upbit.balance_dict[_symbol]['balance']
                        Log(f"{_symbol} 매도")
                        Upbit.매수 = True
                ...
                if close > ema12 and ema12 > ema26:
                    ...
                    if _symbol not in Upbit.balance_dict.keys() and Upbit.not_balance_dict == {} and _symbol not in Upbit.buy_list:
                        ...
                        _bAmount = format(deposit/bid_price,'.8f')
                        _bAmount = float(_bAmount)
                        ...
                        if _symbol in self.symbol_cross_dict.keys():
                            ...
                            if symbol_cross == True:
                                Log(f"{_symbol} 매수")
                                Upbit.매수 = True
                                self.symbol_cross_dict.update({_symbol:False})
                                Upbit.order("BUY", _symbol, _bAmount, bid_price, "")
                ...
                if Upbit.not_balance_dict != {}:
                    ...
                    for i in Upbit.not_balance_dict.copy().keys():
                        ...
                        remaining_volume = float(nbdata["remaining_volume"])
                        remaining_volume = format(remaining_volume,'.8f')
                        remaining_volume = float(remaining_volume)
                        if (bid == "BID") and (remaining_volume > 0.0) and (price < 매수호가):
                            Upbit.order("CANCLE",_symbol, remaining_volume, 매수호가, i)

        # print(Upbit.매수,Upbit.balance_dict)
        # print(self.symbol_cross_dict,Upbit.not_balance_dict,"\n")
