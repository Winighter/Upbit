...
class Upbit:
    ...
    def get_symbol(self):
        sList = []
        symbol_dict = {}
        black_list = ["KRW-TRX","KRW-USDT","KRW-BTC","KRW-STPT","KRW-BIGTIME","KRW-DOGE"]
        ...
        for s in symbols:
            if s.endswith("KRW"):
                ...
                if 1000 >= trade_price and trade_price >= 100:
    ...
    def min_candle_chart(self): # KRW-BTC
        ...
        for _symbol in self.symbol:
            ...
            url = "https://api.upbit.com/v1/candles/days" # n분차트
            querystring = {"market":_symbol,"count":"200"} # count = 필요한 봉 갯수
            response = requests.request("GET", url, params=querystring)
            data = response.json()
                            ...
                            if len(self.balance_dict.keys()) == 1: # 현재로서는 종목 1개만 매매
                                ...
                                if _symbol not in self.balance_dict.keys():
                                    ...
                                    if _symbol not in sym_list:
                                        ...
                                        balance = int(self.balance_dict["KRW-KRW"]["balance"]) # 주문가능금액

                                        # 전부 쓸수는 없기 때문에 나눠서 매매
                                        '''
                                        일단 특정 수치만큼 분할한뒤 (현재는 50%)
                                        '''
                                        deposit = math.trunc(balance*(Upbit.percent_to_be_used/100))
                                        amount = float(format(deposit/bid_price,'.8f'))
                                        if math.trunc(amount*bid_price) < 5000:
                                            pass
                                        else:
                                            print(f"{_symbol} 매수 주문 시작")
                                            Upbit.order("BUY", _symbol, amount, bid_price, "")

                # 취소 및 재주문(재주문은 안되는 것 같음)
                if self.not_balance_dict != {}:

                    for nb in self.not_balance_dict.copy().keys():
                        ...
                        if ask_bid == "BID" and order_type == "limit":
